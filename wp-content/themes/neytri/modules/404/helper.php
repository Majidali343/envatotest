<?php
add_action( 'neytri_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'neytri-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, NEYTRI_THEME_VERSION, 'all');
    }
}

add_filter( 'neytri_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = neytri_404_page_params();
...
function neytri_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'neytri_404_page_params', $params );
}