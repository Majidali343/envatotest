...
if( ! function_exists( 'neytri_shop_woo_loop_product_thumbnail' ) ) {

	function neytri_shop_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
				...
				if( $product->is_featured() ) {
					echo '<div class="featured-tag">
									<div>
										...
										<span>'.esc_html__('Featured', 'neytri').'</span>
									</div>
								</div>';
				}
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_style != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.neytri_html_output($product_overlay_bgcolor).'></div>';
				}
				...
				if($product_show_labels) {
					...
					echo '<div class="product-labels">';
						...
						if( $product->is_on_sale() && $product->is_in_stock() ) {
							echo '<span class="onsale"><span>'.esc_html__('Sale', 'neytri').'</span></span>';
						} else if( !$product->is_in_stock() ) {
							echo '<span class="out-of-stock"><span>'.esc_html__('Sold Out','neytri').'</span></span>';
						}
						...
						if(isset($settings['product-new-label']) && $settings['product-new-label'] == 'true') {
							echo '<span class="new"><span>'.esc_html__('New', 'neytri').'</span></span>';
						}
				...
				if($product_show_offer_percentage) {
					echo neytri_shop_woo_loop_product_offer_percentage($product);
				}


				// Action to run before product thumb image
				do_action( 'neytri_woo_before_product_thumb_image', $product_id );
				...
				if($display_bg_image) {
					...
					echo '<div class="primary-image"><img src="'.esc_url($primary_image_src).'" alt="'.esc_attr__('Shop Primary Image', 'neytri').'" title="'.esc_attr__('Shop Primary Image', 'neytri').'" /></div>';
				}
				...
				if($secondary_image_src != '') {
					if($display_bg_image) {
						...
						echo '<div class="secondary-image"><img src="'.esc_url($secondary_image_src).'" alt="'.esc_attr__('Shop Secondary Image', 'neytri').'" title="'.esc_attr__('Shop Secondary Image', 'neytri').'" /></div>';
					}
				}
			...
			if($product_thumb_content) {
				...
				neytri_shop_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.neytri_html_output($product_thumb_content_overlay_bgcolor).'>';
					do_action('neytri_woo_loop_product_thumb_content', 'thumb');
					remove_all_actions('neytri_woo_loop_product_thumb_content');
				echo '</div>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'neytri_shop_woo_loop_product_thumbnail', 10 );
