...
if( !function_exists('neytri_archive_blog_post_defaults') ) {
    function neytri_archive_blog_post_defaults() {
        $defaults = array(
...
if( !function_exists('neytri_archive_blog_post_misc_defaults') ) {
    function neytri_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('neytri_archive_blog_post_params_default') ) {
    function neytri_archive_blog_post_params_default() {
        $params = array(
            'enable_post_format'   	 => 0,
            'enable_video_audio' 	 => 0,
            'enable_gallery_slider'  => 1,
            'archive_post_elements'  => array( 'feature_image', 'date', 'title', 'content', 'meta_group', 'read_more' ),
            'archive_meta_elements'  => array( 'author', 'comment' ),
            'archive_readmore_text'  => esc_html__('Read More', 'neytri'),
...
if( !function_exists('neytri_archive_blog_post_defaults_filter') ) {
    function neytri_archive_blog_post_defaults_filter() {
        $defaults = neytri_archive_blog_post_defaults();
        return apply_filters( 'neytri_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('neytri_archive_blog_post_misc_defaults_filter') ) {
    function neytri_archive_blog_post_misc_defaults_filter() {
        $defaults = neytri_archive_blog_post_misc_defaults();
        return apply_filters( 'neytri_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('neytri_archive_blog_post_params') ) {
    function neytri_archive_blog_post_params() {
        $params = neytri_archive_blog_post_params_default();
        return apply_filters( 'neytri_archive_blog_post_params', $params );
    }
}

if( !function_exists('neytri_get_archive_post_combine_class') ) {
	function neytri_get_archive_post_combine_class() {

        $blog_defaults = neytri_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'neytri_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('neytri_get_archive_post_holder_class') ) {
	function neytri_get_archive_post_holder_class() {

        $blog_defaults = neytri_archive_blog_post_defaults_filter();
        $blog_misc_defaults = neytri_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'neytri_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('neytri_get_archive_post_style') ) {
	function neytri_get_archive_post_style() {
		...
		return apply_filters( 'neytri_get_archive_post_style', $post_style );
	}
}

if( !function_exists('neytri_get_archive_post_column') ) {
	function neytri_get_archive_post_column() {
		...
		return apply_filters( 'neytri_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'neytri_archive_set_post_class', 10, 3);
if( !function_exists('neytri_archive_set_post_class') ) {
    function neytri_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
			$post_meta = get_post_meta( $post_id, '_neytri_post_settings', TRUE );
            ...
            $blog_params = neytri_archive_blog_post_params();
...
add_action( 'neytri_after_main_css', 'neytri_blog_enqueue_css', 10 );
if( !function_exists( 'neytri_blog_enqueue_css' ) ) {
	function neytri_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, NEYTRI_THEME_VERSION, 'all');

        $post_style = neytri_get_archive_post_style();
        if ( file_exists( get_theme_file_path('/modules/blog/templates/'.$post_style.'/assets/css/blog-archive-'.$post_style.'.css') ) ) {
            wp_enqueue_style( 'wdt-blog-archive-'.$post_style, get_theme_file_uri('/modules/blog/templates/'.$post_style.'/assets/css/blog-archive-'.$post_style.'.css'), false, NEYTRI_THEME_VERSION, 'all');
        }
...
add_action( 'neytri_before_enqueue_js', 'neytri_blog_enqueue_js' );
if( !function_exists( 'neytri_blog_enqueue_js' ) ) {
	function neytri_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = neytri_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'neytri_blog_archive_pagination', neytri_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'neytri_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'neytri_excerpt' ) ) {
	function neytri_excerpt( $limit = NULL ) {
